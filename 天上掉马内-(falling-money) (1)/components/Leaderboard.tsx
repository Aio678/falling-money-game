...
import { ArrowLeft, Medal, Trash2, WifiOff } from 'lucide-react';
...
export const Leaderboard: React.FC<LeaderboardProps> = ({ onBack }) => {
  ...
  const clearLeaderboard = () => {
    if (confirm("确定要清空本地缓存吗？(不会删除数据库记录)")) {
      ...
      <div className="max-w-md w-full flex flex-col h-full bg-white rounded-3xl shadow-xl overflow-hidden border border-indigo-100">
        ...
        <div className="p-4 border-t border-gray-100 flex justify-between bg-gray-50 items-center">
          ...
          <button 
            ...
            className="text-red-400 text-sm flex items-center gap-1 hover:text-red-600 px-3 py-2"
          >
            <Trash2 className="w-4 h-4" /> 清空本地
          </button>
        </div>
      </div>
    </div>
